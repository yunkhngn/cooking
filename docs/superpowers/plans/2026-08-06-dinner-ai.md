...
- Produces: CSS custom properties `--bg --surface --border --ink --ink-muted --teal --teal-soft --teal-tint --coral --coral-tint --radius-card --radius-control --shadow-card --shadow-card-hover`, plus Tailwind `@theme` aliases so `bg-surface`, `text-ink`, `text-ink-muted`, `bg-teal`, `bg-coral`, `border-hairline`, `rounded-card`, `rounded-control`, `shadow-card` are usable in every later task.
...
@theme inline {
  ...
  --color-coral-tint: var(--coral-tint);
...
describe("parsePartial", () => {
  ...
  it("exposes completed dishes while the array is still open", () => {
    const cut = full.indexOf('{ "name": "Canh') > -1
      ? full.indexOf('"Canh')
      : full.indexOf("Canh");
...
export function PreferenceForm({
  ...
  return (
    <form
      ...
      className="rounded-card border border-hairline bg-surface p-6 shadow-[var(--shadow-card)] sm:p-8"
    >
      ...
      <button
        type="submit"
        disabled={disabled}
        className="mt-6 w-full rounded-control bg-coral px-6 py-4 text-base font-semibold text-white shadow-[var(--shadow-card)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
      >
...
export function DishCard({ dish, index }: { dish: Dish; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="overflow-hidden rounded-card border border-hairline bg-surface shadow-[var(--shadow-card)]"
    >
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
...
export function ShoppingList({ list }: { list: DinnerMenu["shoppingList"] }) {
  return (
    <section className="rounded-card border border-hairline bg-surface p-6 shadow-[var(--shadow-card)]">
...
```bash
git add lib/links.ts components/dish-card.tsx components/shopping-list.tsx components/menu-result.tsx lib/__tests__/links.test.ts components/__tests__/dish-card.test.tsx
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 sm:py-16">
      ...
      {status === "error" && (
        <div
          role="alert"
          className="mt-8 rounded-card border border-hairline bg-surface p-6 text-center shadow-[var(--shadow-card)]"
        >
