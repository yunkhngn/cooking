...
- Produces: `streamMenuText(input: GenerateRequest): AsyncIterable<string>` from `lib/gemini.ts`; `POST` handler at `/api/generate` returning a `text/plain` stream or a JSON error.
